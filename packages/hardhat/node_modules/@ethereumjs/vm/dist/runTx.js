...
const debug = debug_1.debug('vm:tx');
const debugGas = debug_1.debug('vm:tx:gas');
/**
 * @ignore
 */
...
async function _runTx(opts) {
    ...
    if (this.DEBUG) {
        debug(`Running tx=0x${tx.isSigned() ? tx.hash().toString('hex') : 'unsigned'} with caller=${caller.toString()} gasLimit=${gasLimit} to=${to ? to.toString() : ''} value=${value} data=0x${util_1.short(data)}`);
    }
    ...
    const cumulativeGasUsed = ((_b = opts.blockGasUsed) !== null && _b !== void 0 ? _b : block.header.gasUsed).add(results.gasUsed);
...
async function generateTxReceipt(tx, txResult, cumulativeGasUsed) {
    ...
    let receipt;
    if (this.DEBUG) {
        debug(`Generate tx receipt transactionType=${tx.type} gasUsed=${cumulativeGasUsed.toString()} bitvector=${util_1.short(baseReceipt.bitvector)} (${baseReceipt.bitvector.length} bytes) logs=${baseReceipt.logs.length}`);
    }
