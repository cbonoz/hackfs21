...
exports.handlers = new Map([
    ...
    [
        ...
        function (runState) {
            ...
            util_1.subMemUsage(runState, memOffset, dataLength);
            if (!dataLength.eqn(0)) {
                runState.eei.useGas(new ethereumjs_util_1.BN(runState._common.param('gasPrices', 'copy')).imul(util_1.divCeil(dataLength, new ethereumjs_util_1.BN(32))), 'CODECOPY opcode');
                const data = util_1.getDataSlice(runState.eei.getCode(), codeOffset, dataLength);
                const memOffsetNum = memOffset.toNumber();
    ...
    [
        ...
        async function (runState) {
            ...
            if (!dataLength.eqn(0)) {
                // copy fee
                runState.eei.useGas(new ethereumjs_util_1.BN(runState._common.param('gasPrices', 'copy')).imul(util_1.divCeil(dataLength, new ethereumjs_util_1.BN(32))), 'EXTCODECOPY opcode');
                const code = await runState.eei.getExternalCode(addressBN);
                const data = util_1.getDataSlice(code, codeOffset, dataLength);
                const memOffsetNum = memOffset.toNumber();
                const lengthNum = dataLength.toNumber();
                runState.memory.extend(memOffsetNum, lengthNum);
                runState.memory.write(memOffsetNum, lengthNum, data);
    ...
    [
        ...
        function (runState) {
            ...
            util_1.subMemUsage(runState, memOffset, dataLength);
            if (!dataLength.eqn(0)) {
                runState.eei.useGas(new ethereumjs_util_1.BN(runState._common.param('gasPrices', 'copy')).mul(util_1.divCeil(dataLength, new ethereumjs_util_1.BN(32))), 'RETURNDATACOPY opcode');
                const data = util_1.getDataSlice(runState.eei.getReturnData(), returnDataOffset, dataLength);
                const memOffsetNum = memOffset.toNumber();
                const lengthNum = dataLength.toNumber();
                runState.memory.extend(memOffsetNum, lengthNum);
                runState.memory.write(memOffsetNum, lengthNum, data);
    ...
    [
        ...
        async function (runState) {
            ...
            if (deductGas) {
                runState.eei.useGas(new ethereumjs_util_1.BN(runState._common.param('gasPrices', 'callNewAccount')), 'SELFDESTRUCT opcode -> callNewAccount');
            }
